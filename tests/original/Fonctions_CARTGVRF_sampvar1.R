...
# draws randomly B bootstrap samples of size N (the size of the sample of data) and retunrs 
# a matrix which with N lines and B columns which contains the indices of the observations 
# belonging each boostrap sample. 

#data : the trainning dataset
#crit : 1=Gini,2=Entropie,3=Misclassification error
#case_min : minimum number of cases/non cases in a node which allows to split the node
#group : a vector with the group number of each variable (WARNING : if there are P goups, then the vector group must contains all the numbers from 1 to P )
#maxdepth : the max depth for a split-tree
#penalty :"No","Size","Root.size" or "Log"
