      ...
      if (improvment[i] > 0){#Is there any group that reduces the node impurity 
        ...
        if(IMPORTANCE==TRUE){
          ...
          imp_sur[-ind_var]<-sapply(igroups[-ind_var],
                                    function(x)unlist(surrogate_split(Ystar,node,group=group,igroup=x,penalty=penalty,label=yval[i],maxdepth=maxdepth)[[crit]]))
...
#' @param tree_seq the object returned by the function "\code{extract_subtrees}". It is a sequence of optimal subtrees obtained by applying
...
#'                 Precisely, the i-th element is the object returned by the function "\code{predict_cartgv}" for the i-th subtree nd by using 
